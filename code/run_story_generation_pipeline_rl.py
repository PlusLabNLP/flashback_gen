        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if 't5' in args.model:
                    ...
                    output = story_model(storylines, attention_mask=storyline_masks,
                                        decoder_input_ids=decoder_output_ids_s)
                    ...
                    #factors = torch.FloatTensor(factors).to(reward.device)
                    loss = loss*reward.unsqueeze(-1)#*factors.unsqueeze(-1)
            ...
            if args.do_eval:
                ...
                for batch in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    with torch.no_grad():
                        # eval_loss = model(input_ids, attention_mask=input_masks, labels=output_ids,
                        #                   decoder_attention_mask=output_masks)[0]
                ...
                #logger.info("BLEU Score is %.4f" % np.mean(calculate_bleu(golds, preds)))
                ...
                # if 'pretrain_with_' in args.output_dir:
                #     pretrain_model_file = os.path.join(args.output_dir, "pytorch_model%s.bin" % epoch)
                #     torch.save(model_to_save.state_dict(), pretrain_model_file)
