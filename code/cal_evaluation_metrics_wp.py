...
def main(args):
    ...
    for filename in args.filenames:
        ...
        if 'test_story_generation_wp_baseline_final' in filename:
            with open('../data/test_story_generation_wp_baseline_final.json') as infile:
                stories = [ex['story'] for ex in json.load(infile)]

            gen_stories = []
            for sents in stories:
                gen_stories.append(' '.join(sents))
                for j in range(4):
                    for ngram in collect_all_ngrams(sents, n=j + 1):
                        counters[j][ngram] += 1
        else:
...
if __name__ == "__main__":
    ...
    args.filenames = ["_content_planning_baseline.json",
                      "_gen_no_struct_pipeline_story_input_wp_max500_topk_sample_5.json",
                      "_gen_with_rel_pipeline_story_input_wp_max500_topk_sample_5.json",
                      "_gen_with_rel_pipeline_story_input_wp_max500_rl_topk_sample_5.json"]
